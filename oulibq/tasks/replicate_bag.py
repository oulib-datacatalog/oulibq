...
def replicate_bag(bag, project=None, department=None, force=None, celery_queue="digilab-nas2-prod-workerq"):
    ...
    if len(bag_chain)==2:        
        cp_val_chain = (bag_chain[0].apply_async(queue=celery_queue) | bag_chain[1].apply_async(queue=celery_queue) | group(subtasks)|clean_nas_files.si().apply_async(queue=celery_queue))()
    elif len(bag_chain)==1:
        cp_val_chain = (bag_chain[0].apply_async(queue=celery_queue) |  group(subtasks)|clean_nas_files.si().apply_async(queue=celery_queue))()
    else:
        cp_val_chain = (group(subtasks) | clean_nas_files.si().apply_async(queue=celery_queue))()
