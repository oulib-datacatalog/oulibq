...
def get_celery_worker_config(api_host):
    ...
    if not os.getenv('REMOTE_BAGIT_SRC_PATH', None) or not os.getenv('LOCAL_BAGIT_SRC_PATH', None) or not os.getenv(
            'REMOTE_BAGIT_DEST_PATH', None):
        raise Exception("Environmental Variables not set!")
    # set config variables
    config = {"s3": {"bucket": "ul-bagit"},
              "nas": {"bagit": os.getenv('REMOTE_BAGIT_SRC_PATH', None),
